	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				//return tex2D(styleMask, float2(pos.x, pos.y) / styleMask_TexelSize.zw);
				...
				for (int x = -vic; x <= vic; x++)
				{
					for (int y = -vic; y <= vic; y++)
					{
						...
						if (isBG(float2(pos.x + x, pos.y + y) / _CameraDepthNormalsTexture_TexelSize.zw)) {
							...
							int styleId = tex2D(styleMask, float2(pos.x + x, pos.y + y) / styleMask_TexelSize.zw) * styleCount;
