			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				for (int x = -vic; x <= vic; x++)
				{
					for (int y = -vic; y <= vic; y++)
					{
						...
						if (tex2D(styleMask, float2(pos.x + x, pos.y + y) / styleMask_TexelSize.zw).r < 0.05f ) {
							//color += _bgColor;
							...
							int styleId = tex2D(styleMask, float2(pos.x + x, pos.y + y) / styleMask_TexelSize.zw).g * styleCount;
